...
- [x] `"OBSTACLE AVOIDANCE"` firing end to end was listed as unconfirmed above -- confirmed 2026-09-22 (see What We Verified), after also finding and fixing the `maze.world` plugin gap below. It does fire and forward correctly.
- [x] `maze.world` (the "Quick start" world) was missing the `Sensors`/`Contact` gz-sim system plugins, so `/scan`, `/imu`, `/camera/*` and `/bump` never published anything through it -- silently, with no error. Fixed by adding the same plugin block `gauntlet_final.world` already has; re-verified `/scan` publishing live. `flatland.world`, `dh.world` and `bod_volcano.world` are missing the same plugins and were not checked.
- [x] `compute_potential_field` produced a maxed-out, flip-flopping turn with no forward motion for an obstacle straight ahead (the atan2 discontinuity described in Behavior 2). Fixed with a fixed 20-degree bias on every repulsive vector; re-verified offline and live.
- [x] Even with that fixed, the robot could get stuck oscillating right at the hard-stop threshold, with `cmd_vel` alternating between a real command and exact `(0, 0)` almost every message (Behavior 2). Fixed with hysteresis (wider thresholds to *release* a stop than to *trigger* one); re-verified live -- real distance to the obstacle grew from 0.4 m to over 1.0 m in 15 s instead of staying flat.
- [x] `collision_avoidance.py`'s `self.bumped` stayed `True` forever after one real touch, so a genuinely blocked-and-then-cleared robot never moved again -- this is what a teammate reported as "doesn't move until the object is moved" (Behavior 2). Same root cause as the `finite_state_controller.py`/`path_following.py` item below and the earlier `safe_teleop.py` fix: the simulator's bump sensor never sends a "cleared" message. Fixed with the same timeout approach; re-verified live: hard-stopped while touching, resumed within 0.3 s of being moved to open space with no new bump messages.
- [x] `finite_state_controller.py` and `path_following.py` had the same `self.bumped`-latches-forever bug as `collision_avoidance.py` above (see the FSM's Capabilities and Limitations). Fixed with the same timeout approach; re-verified `finite_state_controller.py` live (one `/bump` message pinned `/cmd_vel` at zero, then it resumed on its own after 0.3 s with no new bump) and `path_following.py` with a synthetic test against the real class (bump call -> `paused`, then `check_bump_timeout()` alone -> `following` again after 0.3 s).
