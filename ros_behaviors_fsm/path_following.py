"""
Draw and follow a path on the saved map, with a live FSM control panel.

Draw a route on the saved map, or right-click a goal for A* to plan
one, and follow it with pure pursuit. Also hosts the always-visible
FSM control panel (mode buttons, live status, arrow-key driving) this
app opens with.
"""
...
def resample_path(points, spacing):
    """
    Resample the polyline to points spaced `spacing` apart.

    First and last points are kept, so a hand-drawn stroke's uneven
    sampling doesn't affect the follower.
    """
    if len(points) < 2:
        return list(points)
...
def smooth_path(points, passes=2):
    """
    Smooth the path by averaging each interior point with its neighbors.

    Runs for a few passes to take the wobble out of a hand-drawn
    stroke; endpoints are left alone.
    """
    pts = np.asarray(points, dtype=float)
...
class PathFollower(Node):
    """
    Follow a waypoint list with pure pursuit.

    Pauses for a bump, an e-stop, or anything close ahead, and reports
    its status and pose for the drawing window and the gateway FSM to
    watch.
    """

    def __init__(self, ui_queue: Queue):
        """
        Declare tuning parameters and wire up publishers/subscriptions.

        Declares the follower's tuning parameters and wires up its
    ...
    def mode_callback(self, msg):
        """
        Track the gateway FSM's current mode and update the drawing window.

        Logs the new mode and tells the control panel to show or hide
        the drawing window accordingly.
        """
    ...
    def send_fsm_command(self, key):
        """
        Publish the same t/m/g/p key fsm_node's own keyboard listener sends.
    ...
    def drive_teleop(self, linear, angular):
        """
        Drive teleop_scan.py's robot, same as pressing wasd in its terminal.

        Only takes effect while fsm_node is in TELEOP SCAN mode.
        """
        msg = Twist()
    ...
    def save_teleop_map(self):
        """
        Tell teleop_scan.py to save its map now, same as pressing m there.

        Works even if that process has no real terminal.
        """
    ...
    def process_scan(self, msg):
        """
        Flag whether anything is within stop_distance in the front cone.

        Used by control_loop to decide whether to pause.
        """
    ...
    def process_bump(self, msg):
        ...
        check_bump_timeout is what clears it, since the simulator never
        sends an explicit "bump cleared" message.
        """
    ...
    def follow(self, waypoints):
        """Start following the given (x, y) waypoint list, and publish it for RViz."""
        with self.lock:
    ...
    def status_text(self):
        """Return a short human-readable summary for the drawing window's status line."""
        with self.lock:
    ...
    def publish_status(self):
        """Publish the current state (idle/following/paused/done) on path_following_status."""
        with self.lock:
            state = self.state
    ...
    def control_loop(self):
        """
        Pause for a bump, e-stop, or close obstacle; steer otherwise.

        Marks the path done once it's finished.
        """
    ...
    def steer(self):
        """
        Pure pursuit: advance, aim ahead, turn in place, and slow near the goal.

        Advances to the nearest upcoming waypoint, aims at the point
        lookahead meters further along, turns in place for a large
        heading error, and slows down near the goal.
...
class PathPainter:
    ...
    The user can draw strokes on the map, set a goal point, and plan a
    path with A*. The GUI also lets the user clear the strokes, undo
    the last stroke, and reload the map.
    """
    ...
    def __init__(self, node, master):
        """
        Build the drawing window and buttons, load the map, and start hidden.

        Built as a Toplevel under master.
        """
        self.node = node
        self.map = None
        self.strokes = []
    ...
    def reload_map(self):
        ...
        Clears any in-progress drawing.
        """
        try:
    ...
    def redraw(self):
        """
        Repaint the map, every drawn stroke, the waypoints, and the goal marker.

        Waypoints flagged as too close to an obstacle are drawn
        differently.
        """
        if self.map is None:
            return
    ...
    def plan(self):
        """Turn the drawn strokes into a smoothed, evenly-spaced waypoint list."""
        joined = [p for stroke in self.strokes for p in stroke]
    ...
    def _follow_if_valid(self, waypoints, label):
        """
        Check waypoints against the map and start following if clear.

        Updates self.waypoints and self.bad_points and redraws either
        way, then starts following if the whole path is clear.

        Return True if it was accepted and handed to the robot, False
        if any waypoint was too close to an obstacle.
        """
    ...
    def go(self):
        """Follow the drawn path, falling back to an A*-planned route if blocked."""
        if self.map is None:
            return
        waypoints = self.plan()
        if len(waypoints) < 2:
    ...
    def plan_astar(self):
        """Plan and follow an A* route from the robot to the right-clicked goal."""
        if self.map is None:
            return
    ...
    def tick(self):
        """Refresh the status line and redraw the robot marker; reschedule itself."""
        self.status.config(
    ...
    def update_ui(self):
        """Process one iteration of Tkinter events without blocking execution."""
        try:
    ...
    def close(self):
        """
        Handle the window's [X] button: cancel the path and leave PATH FOLLOWING.

        Tells fsm_node to switch modes, same as switching any other way.
        """
        self.node.cancel()
...
class ControlPanel:
    ...
    def __init__(self, node, ui_queue):
        """
        Build the panel's widgets and the (initially hidden) drawing window.

        Builds the mode buttons, status line, drive area, and
        mode-history log, and creates the drawing window under this
        panel's root.
        """
        self.node = node
        self.ui_queue = ui_queue
    ...
    def on_drive_press(self, key):
        """
        Record an arrow key as held, switching to TELEOP SCAN if needed.

        Switches to TELEOP SCAN first if this is the first key pressed
        while in some other mode.
        """
    ...
    def drive_tick(self):
        """
        Compose the held arrow keys into one Twist, and publish it.

        Works the same way teleop_scan.py's own 0.1s timer keeps
        re-sending whatever the last keypress set -- so releasing every
        key stops the robot instead of leaving a stale command in
        flight.
        """
        linear = angular = 0.0
    ...
    def save_map(self):
        """Tell teleop_scan to save its map, and show a self-clearing confirmation."""
    ...
    def tick(self):
        """
        Refresh the status line, log, and drawing window; reschedule itself.

        Appends any new mode-history entries to the log, and
        shows/hides the drawing window on a pending mode change.
        """
    ...
    def on_close(self):
        """
        Handle the panel's own [X] button: cancel, switch modes, and exit.

        Cancels any path in progress, switches back to wall following,
        and exits the Tk main loop.
        """
        self.node.cancel()
        self.node.send_fsm_command('g')
        self.root.quit()
...
def main(args=None):
    """
    Initialize rclpy, run the control panel until closed, then shut down.
