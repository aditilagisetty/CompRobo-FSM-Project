...
class PathFollower(Node):
    ...
    def status_text(self):
        ...
        if state == 'paused':
            reason = ('e-stop' if self.manual_estop else
                      'bumped' if self.bumped else 'obstacle ahead')
    ...
    def steer(self):
        ...
        angular = max(-self.max_angular_speed,
                      min(self.max_angular_speed, self.k_heading * error))
...
class PathPainter:
    ...
    def on_drag(self, event):
        ...
        self.canvas.create_line(last[0], last[1], event.x, event.y,
                                fill='#d62728', width=3, capstyle=tk.ROUND,
                                tags='stroke')
    ...
    def redraw(self):
        ...
        for a, b in zip(joined, joined[1:]):
            self.canvas.create_line(a[0], a[1], b[0], b[1], fill='#d62728',
                                    width=3, capstyle=tk.ROUND, tags='stroke')
        if joined:
            x, y = joined[0]
            self.canvas.create_oval(x - 6, y - 6, x + 6, y + 6, fill='#2ca02c', outline='')
            x, y = joined[-1]
            self.canvas.create_rectangle(x - 6, y - 6, x + 6, y + 6, fill='#d62728', outline='')
        for wx, wy in self.waypoints:
            x, y = self.world_to_canvas(wx, wy)
            self.canvas.create_oval(x - 2, y - 2, x + 2, y + 2, fill='#ff7f0e', outline='')
    ...
    def tick(self):
        self.status.config(text=f'{self.node.status_text()}   '
        ...
        if self.map is not None and self.node.have_odom:
            ...
            self.canvas.create_oval(x - r, y - r, x + r, y + r, outline='#1f77b4',
                                    width=2, tags='robot')
...
def main(args=None):
    ...
    try:
        ...
        print(f'No map found at {node.map_file}.\n'
