# Day 4 annotation (single-threaded sample):
#
# - run_loop is driven by a 0.1s timer, so it's called at 10Hz. Nothing in
#   run_loop sleeps or blocks -- it only compares clock timestamps and
#   publishes -- so it always returns immediately and never blocks the
#   executor.
# - FSM: two states, "driving straight" (executing_turn=False) and
